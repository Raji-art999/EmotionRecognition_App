...
void detectFaceAndPredictEmotion(cv::dnn::Net &net, cv::CascadeClassifier &face_cascade, dlib::shape_predictor &pose_model) {
    while (!stopFlag) {
        cv::Mat localFrame;
        ...
        for (const auto &face : faces) {
            ...
            // Convert the cv::Mat to dlib's image format
            dlib::cv_image<unsigned char> dlibImg(faceROI);
            dlib::rectangle rect(face.x, face.y, face.x + face.width, face.y + face.height);
            dlib::full_object_detection shape = pose_model(dlibImg, rect);

            // Draw landmarks
            for (int i = 0; i < shape.num_parts(); i++) {
                cv::circle(localFrame, cv::Point(shape.part(i).x(), shape.part(i).y()), 2, cv::Scalar(0, 255, 0), -1);
            }
...
int main() {
    ...
    cv::dnn::Net net = cv::dnn::readNetFromTensorflow("/Users/kellerrajashree/Desktop/OpenCV_C++/frozen_model_new_24102024.pb");
    if (net.empty()) {
    ...
    dlib::deserialize("/Users/kellerrajashree/Desktop/OpenCV_C++/shape_predictor_68_face_landmarks.dat") >> pose_model;
    ...
    std::thread emotionThread(detectFaceAndPredictEmotion, std::ref(net), std::ref(face_cascade), std::ref(pose_model));
